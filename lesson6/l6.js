...
function haveBreakfest(isEat, cb) {
    setTimeout(() => {
        if (isEat) {
            cb(null, 'Grechka ))))))))')
        } else {
            cb('Znov budu golodna ((')
        }
    }, 1000)
}

function goToMarshrutka(money, cb) {
    setTimeout(() => {
        if (money >= 7) {
            cb(null, 'Ура ура ура. Я БАГАТА')
        } else {
            cb('Йду пішки (((')
        }
    }, 3000)
...
myDay(true, (err, data) => {
    if (err) {
        ...
        console.log(data);
        haveBreakfest(true, (err, data) => {
            if (err) {
                console.log(err, 'breakfest');
            } else {
                console.log(data);
                goToMarshrutka(5, (err, data) => {
                    if (err) {
                        console.log(err, 'Marsh');
                    } else {
                        console.log(data);
                        console.log('FINISH. Klimash ((');
                    }
                })
            }
        })
